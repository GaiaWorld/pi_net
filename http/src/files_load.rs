...
impl<S: Socket, W: AsyncIOWait> Middleware<S, W, GatewayContext> for FilesLoad {
    fn request<'a>(&'a self, context: &'a mut GatewayContext, req: HttpRequest<S, W>)
        ...
        let future = async move {
            ...
            //异步加载所有文件
            match async_load_files(&resp, dir_vec) {
                Err(e) => {
...
fn async_load_files<S: Socket, W: AsyncIOWait>(resp: &HttpResponse<S, W>, files: Vec<(u64, PathBuf)>) -> Result<(u64, usize)> {
                            ...
                            match result {
                                ...
                                Ok(bin) => {
                                    ...
                                    if let Err(e) = resp_handler_copy.write_index(index, bin) {
                                        ...
                                        //发送文件成功，则减去当前文件的大小
                                        let last_size = unload_size_copy.fetch_sub(bin_size, Ordering::Relaxed);
