...
impl<S: Socket> MqttConnectHandle<S> {
    ...
    /// 获取连接会话上下文的只读引用
    pub fn get_session(&self) -> Option<ContextHandle<QuicBrokerSession>> {
        if self.is_closed.load(Ordering::Relaxed) {
            return None;
        }
    ...
    pub fn close(&self, code: u32, reason: Result<()>) -> Result<()> {
        ...
        self.connect.close(code, reason)
    }
}
...
impl<S: Socket> MqttBrokerListener<S> for MqttProxyListener {
    ...
    fn closed(&self,
        ...
        //Mqtt连接已关闭
        if let Err(e) = &reason {
            warn!("Mqtt proxy connect close by error, uid: {:?}, remote: {:?}, local: {:?}, reason: {:?}",
                connect.get_uid(),
