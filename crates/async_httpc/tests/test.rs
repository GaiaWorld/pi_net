...
use pi_async_rt::rt::{AsyncRuntime, AsyncRuntimeBuilder as RTAsyncRuntimeBuilder, startup_global_time_loop,
...
fn test_multi_request() {
    let _handle = startup_global_time_loop(100);
    let rt = RTAsyncRuntimeBuilder::default_multi_thread(None, None, Some(1), None);
    ...
    for _ in 0..100 {
        ...
        rt.spawn(async move {
            let now = Instant::now();
            match httpc_copy
                .build_request("https://ccfg.17youx.cn/srs_boom_test/local/XZXpaXDk6k4BV4XGpVYdCm/827674/assets_server_android/cfg", AsyncHttpRequestMethod::Get)
                .send().await {
                Err(e) => println!("!!!!!!test request failed, e: {:?}", e),
                Ok(mut resp) => {
                    println!("!!!!!!request time: {:?}", now.elapsed());
                    ...
                    loop {
                        match resp.get_body().await {
                            Err(e) => {
                                panic!("!!!!!!get body failed, reason: {:?}", e);
                            },
                            Ok(Some(body)) => {
                                body_len += body.len();
                                continue;
