...
impl QuicSocket {
    ...
    pub fn send_transmit(&self, data: Transmit) -> Result<()> {
        if let Some(size) = data.segment_size {
            ...
            //单帧Quic数据报文
            let len = data.contents.len(); //数据报文的大小
    ...
    pub fn recv(&mut self) -> Result<usize> {
        ...
        match self.connect.recv_stream(self.get_main_stream_id().unwrap().clone()).read(false) {
            ...
            Ok(mut chunks) => {
                ...
                let _ = chunks.finalize(); //完成本次流的所有数据的接收
    ...
    pub fn read_ready(&mut self, adjust: usize) -> GenResult<AsyncValueNonBlocking<usize>, usize> {
        ...
        let remaining = self.read_buf.lock()
                .as_ref()
                .unwrap()
                .remaining();
    ...
    pub fn write_ready<B>(&mut self, buf: B) -> Result<()>
        ...
        if let Some(sender) = &self.event_send {
            let _ = sender.send(QuicEvent::StreamWrite(self.handle, None, buf.as_ref().to_vec()));
        }
