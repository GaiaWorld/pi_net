...
use driver::{Socket, Stream, SocketAdapter, SocketOption, SocketConfig, SocketDriver};
...
fn handle_accepted<S: Socket + Stream, A: SocketAdapter<Connect = S>>(pool: &mut TcpSocketPool<S, A>) {
    ...
    for mut socket in pool.socket_recv.try_iter().collect::<Vec<S>>() {
        ...
        if let Err(e) = pool.poll.register(socket.get_stream(), token, socket.get_ready().clone(), socket.get_poll_opt().clone()) {
            ...
            pool.driver.as_ref().unwrap().get_adapter().connected(Ok(&mut socket)); //执行连接回调
            entry.insert(socket); //加入连接池上下文
        }
    }
}
...
fn init_socket<S: Socket + Stream, A: SocketAdapter<Connect = S>>(socket: &mut S,
    ...
    if socket.get_local().is_ipv6() {
        ...
        stream.set_only_v6(true);
    }

    //设置连接通用选项
    stream.set_recv_buffer_size(socket_opts.recv_buffer_size);
    stream.set_send_buffer_size(socket_opts.send_buffer_size);
...
fn handle_wakeup_readable<S: Socket + Stream, A: SocketAdapter<Connect = S>>(pool: &mut TcpSocketPool<S, A>) {
    ...
    for token in pool.wakeup_readable_recv.try_iter().collect::<Vec<Token>>() {
        if let Some(socket) = pool.contexts.get_mut(token.0) {
            pool.poll.reregister(socket.get_stream(), token, socket.get_ready().clone(), socket.get_poll_opt().clone());
        }
    }
}
            ...
            if ready.is_readable() {
                ...
                match socket.recv() {
                    ...
                    Ok(len) => {
                        ...
                        pool.poll.reregister(socket.get_stream(), token, socket.get_ready().clone(), socket.get_poll_opt().clone());
                        pool.driver.as_ref().unwrap().get_adapter().readed(Ok(socket));
                    },
                ...
                match socket.send() {
                    ...
                    Ok(len) => {
                        ...
                        pool.poll.reregister(socket.get_stream(), token, socket.get_ready().clone(), socket.get_poll_opt().clone());
                        pool.driver.as_ref().unwrap().get_adapter().writed(Ok(socket));
                    },
                    Err(ref e) if e.kind() == ErrorKind::WouldBlock => {
...
fn close_socket<S: Socket + Stream, A: SocketAdapter<Connect = S>>(pool: &mut TcpSocketPool<S, A>, token: Token, reason: Result<()>) {
    let mut socket = pool.contexts.remove(token.0); //从Tcp连接池上下文中，移除指定令牌的Tcp连接

    pool.map.remove(socket.get_remote()); //从Tcp连接池的对端地址映射表中，移除Tcp连接的令牌

    //从轮询器中注销Tcp连接
    if let Err(e) = pool.poll.deregister(socket.get_stream()) {
        //从轮询器中注销Tcp连接失败
        println!("!!!Tcp Socket Unregister Error, token: {:?}, remote: {:?}, local: {:?}, reason: {:?}", token, socket.get_remote(), socket.get_local(), e);
    }
    ...
    if let Err(e) = reason {
        ...
        pool.driver.as_ref().unwrap().get_adapter().closed(Err((&mut socket, e)));
    } else {
        //正常关闭Tcp连接，则先执行关闭回调，再关闭Tcp连接
        pool.driver.as_ref().unwrap().get_adapter().closed(Ok(&mut socket));
